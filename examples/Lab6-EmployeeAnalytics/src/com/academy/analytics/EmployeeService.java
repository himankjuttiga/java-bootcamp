...
public class EmployeeService {
    ...
    public void demonstrateSorting() {
        ...
        employees.stream().sorted(Comparator.comparingInt(Employee::getExperience).reversed())
                .limit(5).forEach(System.out::println);
    }

    public void displayDistinctDepartments() {
        System.out.println("Unique Departments:");
        employees.stream().map(Employee::getDepartment).distinct().sorted()
                .forEach(System.out::println);
    }
    ...
    public void demonstrateCollectors() {
        List<Employee> active = employees.stream()
                .filter(Employee::isActive).collect(Collectors.toList());
    ...
    public String collectEmployeeSummary() {
        return employees.stream().collect(Collector.of(
                StringBuilder::new,
                (sb, e) -> {
                    if (sb.length() > 0) {
                        sb.append(", ");
