...
public class ApiExceptionHandler {
  ...
  /**
   * Object-level authorisation refusal (lab40-001). 403 with no detail about the customer:
   * confirming that CUS-1001 exists but belongs to someone else is still disclosure. The
   * correlation id goes to the log so an operator can reconstruct who asked for what.
   */
  @ExceptionHandler(AccessDeniedException.class)
  public ProblemDetail accessDenied(AccessDeniedException ex, HttpServletRequest request) {
    String correlationId = correlationId(request);
    log.warn("access denied correlation={} path={}", correlationId, request.getRequestURI());
    return problem(
        HttpStatus.FORBIDDEN,
        "Forbidden",
        "That customer is not assigned to you.",
        correlationId);
  }
